...
do
    if [[ $1 == -ld_debug* ]];  then
        ...
        echo "OSTYPE: $OSTYPE"
        shift
    elif [[ $1 == -execprefix=* ]];  then
        echo "Option: $1"
        WISK_EXECPREFIX=${1##*-execprefix=}
...
time env -i strace -E LD_PRELOAD="$LD_PRELOAD" -ff -v -s 1024 -q -o $STRACEDIR/strace.log -E RUST_BACKTRACE="$RUST_BACKTRACE" -E TERM="$TERM" -E HOME="$HOME" -E PATH="$PATH" -E USER="$USER" -E WISK_TRACE="$WISK_TRACE" -E WISK_TRACK="$WISK_TRACK" -E WISK_CONFIG="$WISK_CONFIG" -E WISK_WSROOT="$WISK_WSROOT" -E WISK_EXECPREFIX="$WISK_EXECPREFIX" "$@"
elif [[ ! -z $DOPERF ]]; then
time env -i RUST_BACKTRACE="$RUST_BACKTRACE" TERM="$TERM" HOME="$HOME" LD_PRELOAD="$LD_PRELOAD" PATH="$PATH" USER="$USER" WISK_TRACE="$WISK_TRACE" WISK_TRACK="$WISK_TRACK" WISK_CONFIG="$WISK_CONFIG" WISK_WSROOT="$WISK_WSROOT" WISK_EXECPREFIX="$WISK_EXECPREFIX" perf record -g $@
else
time env -i RUST_BACKTRACE="$RUST_BACKTRACE" TERM="$TERM" HOME="$HOME" LD_PRELOAD="$LD_PRELOAD" PATH="$PATH" USER="$USER" WISK_TRACE="$WISK_TRACE" WISK_TRACK="$WISK_TRACK" WISK_CONFIG="$WISK_CONFIG" WISK_WSROOT="$WISK_WSROOT" WISK_EXECPREFIX="$WISK_EXECPREFIX" "$@"
fi
